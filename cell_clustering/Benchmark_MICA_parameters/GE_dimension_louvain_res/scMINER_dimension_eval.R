...
for (dataset in datasets_id ){
  # heatmap
  summary_file <- list.files(pattern = paste0(dataset,'.*summary.csv'))
  eval_df <- read.table(summary_file)
  #eval_df <- eval_df[eval_df$louvain_res <=3, ]
  mat <- reshape2::acast(eval_df, component ~ louvain_res, value.var = "ARI")
  ...
  pdf(paste0(dataset,'_scMINER_sweep_dimension_Heatmap.pdf'))
  p <- Heatmap(mat,
