...
# conda activate mica_100

input_dir='/home/xyao/Dataset/scMINER_cell_clustering_13datasets_h5ad_input'
output_root='/home/xyao/MICA/mica_100/bin_size_dimension_sweeping/GE_bin_size_louvain_res'
...
for h5ad_file in $(ls $input_dir);do
	...
	do  
		...
		output_dir="${output_root}/${dataset_id}/${dataset_id}_bin_size_${bin_size}"
		#echo $output_dir
		...
		if [[ $mode == 'mds' ]];then
			MICA_CMD="mica mds -i $MICA_input -o $output_dir -pn ${dataset_id} -nck ${n_cluster[${dataset_id}]} -bsz ${bin_size}"
		elif [[ $mode == 'ge' ]]; then
			MICA_CMD="mica ge -i $MICA_input -o $output_dir -pn ${dataset_id} -nw $n_core \
			-nnt ann -bsz ${bin_size} -ir 0.1 -ar 4 -ss 0.2"
		else
