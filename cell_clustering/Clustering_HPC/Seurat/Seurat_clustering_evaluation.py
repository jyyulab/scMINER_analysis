...
for each in ordered_dataset_ids:
    ...
    for label_file in glob.glob(each + '/*louvain*.txt'):
        parts = label_file.split("_")  
      
        louvain = parts[2]+'_'+parts[3] 
        k_value = parts[-1].split('.')[0]
        pred_k = int(k_value[1:])

        tmp_ari = eval_df.loc[eval_df['res'] == louvain,'ARI'].values[0]
    ...
    metric_row = list(eval_df.loc[eval_df['res'] == best_louvain].values[0])[1:]
...
best_pred_label_df.to_csv('Seurat_clustering_evaluation.csv',sep='\t',header=True)
