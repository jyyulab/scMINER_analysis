...
for each in ordered_dataset_ids:
    #print(each)
    if glob.glob(each + '/Silouette_sc3s*'):
        sil_file = glob.glob(each + '/Silouette_sc3s*')[0]
        #print(sil_file)
        pred_k = int(sil_file.split('_')[2])
    else:
        continue
...
best_pred_label_df.to_csv('SC3s_clustering_evaluation.csv',sep='\t',header=True)