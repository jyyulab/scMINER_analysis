...
def main():
    ...
    np.savetxt(args.save_dir + "/final_latent_layer.txt", latent_layer, delimiter="\t")
    ...
    filename =  args.save_dir + '/scVI_Umap_k'+str(k)+ '.pdf'
    ...
    df['UMAP_X'] = X_coord
    df['UMAP_Y'] = Y_coord
    df['scVI_label'] = adata.obs['leiden'].values


    k = len(set(adata.obs['leiden']))# number of cluster
    df_file = args.save_dir + '/scVI_clustering_k' + str(k) + '.txt'
